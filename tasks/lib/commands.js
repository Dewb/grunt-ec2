...
//        );
	...
	return util.format('%s || sudo %s pm2 start %s/%s -x --name %s --interpreter bash || echo "pm2 already started."',
			   running, parse.toPairs(env), conf('SRV_CURRENT'), conf('BASH_SCRIPT'), name
			  );
